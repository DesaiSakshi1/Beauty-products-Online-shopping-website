      ...
      $pid = $fetch_cart['pid'];
         ...
         if ($select_products->rowCount() > 0) {
            while ($fetch_product = $select_products->fetch(PDO::FETCH_ASSOC)) {
               $update_qty = $fetch_product['quantity'] - $fetch_cart['quantity'];
               $update_quantity = $conn->prepare("UPDATE products SET quantity = ? WHERE id = ?");
               $update_quantity->execute([$update_qty, $pid]);
            }
         }
      }
...
<section class="checkout-orders">
   ...
   <h3>your orders</h3>
      ...
      <div class="flex">
         ...
         <div class="inputBox">
            <span>city :</span>
            <input type="text" name="city" placeholder="e.g. Bhaktapur" class="box" maxlength="50" required>
         </div>
